...
#######################################################################################
#cfilepath <- "r-parsers/20220823_fl_primary_defs.csv"
zindir  <- "r-parsers/input2022/" # full path or path relative to working directory
zoutdir <- "r-parsers/outdos2022/" # full path or path relative to working directory
filestate <- "FL"
#######################################################################################
...
parsefile <- function(county, state, xx){
    # xx <- read_csv(cfile)
    # infile <- paste0(zindir,"20220823_",tolower(state),"_",tolower(county),".csv")
    # write_csv(xx, infile)
    got_ushouse <- FALSE
    ...
    if (gotxx){
        ...
        f_std <- paste0("20220823__",tolower(state),"__primary__",adj_county,"__precinct.csv")
...
for (i in 1:length(cc)){
    zcounty <<- cc[i]
    dd <- read_delim(paste0(zindir,"2022-pri-outputofficial/",
                            cc[i],"_PctResults20220823.txt"), delim = '\t', quote = "",
    ...
    lastParty <- "UNK"
    for (j in 1:NROW(xx)){
        if (str_trim(xx$Party[j]) == ""){
            xx$Party[j] <- lastParty
        }
        else{
            lastParty <- xx$Party[j]
        }
    }
    yy <- as.data.frame(yy)
    #names(yy) <- c("COUNTY","AREA","Contest","DIST","Party","Candidate","Votes")
    xx <- rbind(xx,yy)
