...
dos2018 <- "C:\\Users\\bdavi\\OneDrive2\\github\\oe\\openelections-data-fl\\r-parsers\\outdos2018\\"
indir   <- paste0(dos2018)
outfile <- paste0(dos2018,election,"__",election_area,".csv")
ff      <- paste0(dos2018,election,"__",election_area,".txt")
list <- list.files(indir, pattern = epattern, full.names = FALSE)
len <- length(list)
catfile(ff,epattern)
catfile(ff,paste0("len=",len))
# list <- c("20181106__tx__general__andrews__precinct.csv",
#           "20181106__tx__general__brown__precinct.csv") #create test file
# list <- c("20181106__tx__general__andrews__precinct.csv",
#           "20181106__tx__general__maverick__precinct.csv") #create test file
catfile(ff,list)
